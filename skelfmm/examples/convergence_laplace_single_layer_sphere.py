"""Laplace single-layer sphere convergence example."""
...
def dense_single_layer_potential(targets, points, normals, weights, density, param):
    del normals
    return operators.LAPLACE_3D.numpy_matrix(targets, points, param=param) @ (weights * density)
...
def main():
    ...
    print_sphere_kernel_summary(
        "Laplace single-layer on the sphere",
        report,
        bench,
        fast_self_apply_error(),
